...
void LCD_DisplayStringLine(uint8_t Line, uint8_t *ptr)
{
  ...
  while (i < 20)
  {
    ...
    if(i==18 && *ptr==9)
      LCD_DisplayChar(Line, refcolumn,66);
    else if(i==19 && *ptr==9 && *(ptr-1)==9)
      LCD_DisplayChar(Line,refcolumn,73);
    else if(i==19 && *ptr==8 && *(ptr-1)==9)
      LCD_DisplayChar(Line,refcolumn,48);
    else
      LCD_DisplayChar(Line, refcolumn, *ptr + 0x30);
...
/**
  * @brief  Displays the Count (16dots width, 24dots height).
  * @param Line: the Line where to display the character shape .
  *   This parameter can be one of the following values:
  * @arg Linex: where x can be 0..9
  * @param Column: start column address.
  ...
  * @retval : None
  */
void LCD_DisplayCount(uint8_t Line, uint16_t Column,uint8_t ColorKey, uint8_t Ascii)
{
  if(ColorKey==0)
  {
